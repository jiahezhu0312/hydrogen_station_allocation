...
def scenario_2(cn1, x):
    w_x = x.copy()
    res_air = dict()
    res_competitors = dict()
    for years in [10, 15, 20]:
        ...
        for _ in range(int(0.25 * 200)):
            ...
            if random.random() < 0.5:
                res_air[best_node] = h2station_nodes[best_node]
            else:
                res_competitors[best_node] = h2station_nodes[best_node]
            df_dist = (
                (df_coor.x - df_coor.loc[best_node, "x"]) ** 2
                + (df_coor.y - df_coor.loc[best_node, "y"]) ** 2
            ).pow(0.5)
            df_nearest = df_dist[df_dist <= 40000].index.values
            for key in df_nearest:
                w_x.pop(key, None)
                h2profit_nodes.pop(key, None)
            # remove neighbors from x and h2profit_nodes
        nx.set_node_attributes(cn1, 0, "competitor")
        nx.set_node_attributes(cn1, res_competitors, "competitor")
        nx.set_node_attributes(cn1, 0, "airliquide")
        nx.set_node_attributes(cn1, res_air, "airliquide")
        visualize_network_scenario2(cn1)
